...
static void
parse_inline(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t size)
{
	size_t i = 0, end = 0;
...
parse_math(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size, const char *end, size_t delimsz, int displaymode)
{
	...
	i += delimsz;
	if (delimsz == 2 && !(doc->ext_flags & MARKDOWN_EXT_MATH_EXPLICIT))
...
static size_t
char_linebreak(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
...
char_escape(markdown_buffer *ob, markdown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	...
	if (size > 1) {
		if (data[1] == '\\' && (doc->ext_flags & MARKDOWN_EXT_MATH) &&
		...
		markdown_buffer_putc(ob, data[0]);
	}
